...
test('Template Cost Related Search flow', async ({ page }) => {
  await page.goto('/listing/template/costTempl/vendorChargesTemplateView');
  ...
  if (!hasLink) {
    throw new Error('No records found in Template Cost Related listing — marking as FAILED');
  }
