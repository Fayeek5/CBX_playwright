...
test('Admin Application Configuration Notification Profiles Search flow', async ({ page }) => {
  await page.goto('/');
  ...
  // Click "Notification Profile" sub-item — falls back to "Notifications" if not present (UAT)
  await page.evaluate(() => {
    const btns = Array.from(document.querySelectorAll('button'));
    const btn =
      btns.find(b => {
        const t = b.textContent?.trim() ?? '';
        return t.includes('Notification Profile') && !t.includes('Application Configuration');
      }) ??
      btns.find(b => {
        const t = b.textContent?.trim() ?? '';
        return t.startsWith('Notifications') && !t.includes('Application Configuration');
      });
    (btn as HTMLButtonElement)?.click();
  });
  await page.waitForLoadState('domcontentloaded');
  ...
  console.log('Notification Profiles page URL:', page.url());
  ...
  if (!hasLink) {
    throw new Error('No records found in Application Configuration Notification Profiles — marking as FAILED');
  }
  ...
  console.log('Searching notification profile:', value);
