...
test('Admin Application Configuration Events Search flow', async ({ page }) => {
  await page.goto('/');
  ...
  // Click "Events" sub-item (JS click to bypass overlay)
  await page.evaluate(() => {
    const btn = Array.from(document.querySelectorAll('button'))
      .find(b => {
        const t = b.textContent?.trim() ?? '';
        return t.includes('Events') && !t.includes('Application Configuration');
      });
    (btn as HTMLButtonElement)?.click();
  });
  await page.waitForLoadState('domcontentloaded');
  ...
  if (!hasLink) {
    throw new Error('No records found in Application Configuration Events — marking as FAILED');
  }
  ...
  console.log('Searching event:', value);
