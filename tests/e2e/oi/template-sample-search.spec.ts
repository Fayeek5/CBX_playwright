...
test('Template Sample Related - no records', async ({ page }) => {
  ...
  const subLink = page.getByRole('link', { name: 'Sample Related' });
  const subLinkVisible = await subLink.waitFor({ state: 'visible', timeout: 8000 }).then(() => true).catch(() => false);
  if (!subLinkVisible) { console.log('Sample Related link not found — skipping'); return; }
  await subLink.click();
  await page.waitForLoadState('domcontentloaded');

  await expect(page.getByText('noRecordsFound')).toBeVisible({ timeout: 15000 });
  console.log('Sample Related: noRecordsFound confirmed');
});
