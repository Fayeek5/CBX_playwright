...
test('Library Colors Search flow', async ({ page }) => {
  await page.goto('/');
  ...
  // Click Colors link
  await page.getByRole('link', { name: 'Colors' }).first().click();
  await page.waitForLoadState('domcontentloaded');
  ...
  console.log('Colors page URL:', page.url());
  ...
  if (!hasLink) {
    throw new Error('No records found in Library Colors — marking as FAILED');
  }
  ...
  console.log('Searching color:', value);
