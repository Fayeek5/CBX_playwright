...
test('Template Product Related Search flow', async ({ page }) => {
  await page.goto('/listing/template/productTempl/materialAttrTemplView');
  ...
  const hasLink = await nameLink.waitFor({ state: 'visible', timeout: 30000 }).then(() => true).catch(() => false);
  if (!hasLink) {
    throw new Error('No records found in Template Product Related listing — marking as FAILED');
  }
